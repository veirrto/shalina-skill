...
const filtered = stops.filter(item => {
  return item.IsPublic === true && (item.Zone === 100);
});
...
console.log(`The nearest bus stop is ${dist} meters away: ${nearestStop.Name} (${nearestStop.Latitude}, ${nearestStop.Longitude})`); 
...
const params = {
  ...
  anyObjFilter_sf: 'STOP',
  radius: '1000', // search radius in meters
  coord: `${longitude},${latitude}`,
};
/*
// Create the request URL with the parameters
const url = new URL(apiEndpoint);
Object.keys(params).forEach(key => url.searchParams.append(key, params[key]));
...
fetch(url, { headers })
  ...
  .then(data => {
    ...
    const stopDistance = stop.distance;
    const stopLatitude = stop.lat;
    const stopLongitude = stop.lon;
    
    // Print the result
    console.log(`The nearest bus stop is ${stopDistance} meters away: ${stopName} (${stopLatitude}, ${stopLongitude})`);
  })
  .catch(error => console.error(error));
  */
